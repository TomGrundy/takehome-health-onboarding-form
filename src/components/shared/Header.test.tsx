...
describe('Header', () => {
  ...
  it('should show logout button and user info when user is logged in', () => {
    ...
    localStorage.setItem(STORAGE_KEYS.ONBOARDING_STATE, JSON.stringify(initialState));
    render(<Header />, { wrapper });
  ...
  it('should handle logout and reset state', async () => {
    ...
    await waitFor(() => {
      ...
      expect(contextState.step).toBe(STEPS.LOGIN);
    ...
    if (savedState) {
      ...
      expect(parsed.step).toBe(STEPS.LOGIN);
